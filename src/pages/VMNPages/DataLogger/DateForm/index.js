...
export class DateForm extends Component {
    ...
    openNewTabCsv = (e)=>{
        ...
        validateFields((err, values) => {
            ...
            const link = `http://${window.location.hostname}:3000/vmn-logger/interval/csv?interval=${interval}&start=${startStr}&end=${endStr}&station=${station}`;
    ...
    getStations = ()=>{
        return [0,1,2,3,4,5,6,7,8].map( val => (
            <Option value={val} key={`sta${val}`}>{val}</Option>
        ))
    }
    ...
    getIntervals = ()=>{
        return [1,5,15,30].map( val => (
            <Option value={val} key={`intv${val}`}>{val} minute</Option>
        ))
    }
                        ...
                        <div className="card-body">
                            <Form onSubmit={this.onSubmit}>
                                ...
                                <div className="d-flex justify-content-end">
                                    <Button onClick={this.openNewTabCsv} style={{margin: '5px'}} icon="download">.CSV</Button>
